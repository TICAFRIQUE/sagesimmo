...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-md-8">
            <h1 class="h3 mb-0 text-gray-800">
                <i class="fas fa-money-check-alt"></i> Créer un Versement
            </h1>
        </div>
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('backend.versements.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="proprietaire_id" id="proprietaire_id" class="form-select @error('proprietaire_id') is-invalid @enderror" required>
                            ...
                            @foreach($proprietaires as $prop)
                                <option value="{{ $prop->id }}" @selected(old('proprietaire_id', $proprietairePreselecte ?? null) == $prop->id)>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <small class="text-muted">Revenue net de la période actuelle</small>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <div class="btn-group w-100" role="group">
                            <input type="radio" class="btn-check" name="type_versement" id="typeTotal" value="total" @checked(old('type_versement') == 'total')>
                            <label class="btn btn-outline-primary w-50" for="typeTotal">
                                <i class="fas fa-check-circle me-1"></i> Montant Total
                            </label>
                            
                            <input type="radio" class="btn-check" name="type_versement" id="typePartiel" value="partiel" @checked(old('type_versement') == 'partiel')>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="montant" id="montant" class="form-control @error('montant') is-invalid @enderror" 
                            value="{{ old('montant') }}" placeholder="0" step="1" required>
                        @error('montant')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" name="date_versement" id="date_versement" class="form-control @error('date_versement') is-invalid @enderror" 
                            value="{{ old('date_versement', now()->format('Y-m-d')) }}" required>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="mode_versement" id="mode_versement" class="form-select @error('mode_versement') is-invalid @enderror" required>
                            <option value="">-- Sélectionner --</option>
                            <option value="virement" @selected(old('mode_versement') == 'virement')>Virement</option>
                            <option value="chèque" @selected(old('mode_versement') == 'chèque')>Chèque</option>
                            <option value="espèces" @selected(old('mode_versement') == 'espèces')>Espèces</option>
                            <option value="mobile_money" @selected(old('mode_versement') == 'mobile_money')>Mobile Money</option>
                            <option value="autre" @selected(old('mode_versement') == 'autre')>Autre</option>
                        </select>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="reference" id="reference" class="form-control @error('reference') is-invalid @enderror" 
                            value="{{ old('reference') }}" placeholder="N° virement, chèque...">
                        @error('reference')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    dateDebutInput.value = dateDebutInput.value || firstDay.toISOString().split('T')[0];
    dateFinInput.value = dateFinInput.value || lastDay.toISOString().split('T')[0];
