...
@section('css')
    <style>
        ...
        .echeance-detail-cell { background-color: #f8f9fa; padding: 15px !important; }
        .btn-expand { cursor: pointer; transition: transform 0.2s; }
        .btn-expand.rotated { transform: rotate(90deg); }
        ...
        @media print {
            ...
            .echeance-detail-row { display: none !important; }
        }
    </style>
@endsection
        ...
        <div class="row mb-3 align-items-center">
            <div class="col-md-7">
                ...
                <h5 class="text-muted mb-0">
                    ...
                    @endif
                    @if($locataire->telephone)
                        <small class="ms-2"><i class="fas fa-phone fa-xs"></i> {{ $locataire->telephone }}</small>
                    @endif
                </h5>
            ...
            <div class="col-md-5 text-end">
                ...
                <a href="{{ route('backend.rapports.locataire.pdf', array_filter(['locataire_id' => $locataire->id, 'date_debut' => $dateDebut?->format('Y-m-d'), 'date_fin' => $dateFin?->format('Y-m-d')])) }}" class="btn btn-success btn-sm me-2 no-print" title="Télécharger en PDF">
            ...
            <div class="card-body py-3">
                <form method="GET" action="{{ route('backend.rapports.locataire') }}" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="locataire_id" class="form-select form-select-sm">
                            @foreach ($locataires as $loc)
                                <option value="{{ $loc->id }}" @selected($loc->id == $locataire->id)>{{ $loc->username }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3 d-flex gap-2">
                        ...
                        <a href="{{ route('backend.rapports.locataire') }}" class="btn btn-outline-secondary btn-sm"><i class="fas fa-redo"></i></a>
                    </div>
                </form>
            </div>
        </div>
            ...
            <div class="col-md-2 col-sm-4 mb-2">
                <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #ffc107 !important;">
                    <div class="card-body py-2 text-center">
                        ...
                        <small class="text-danger" style="font-size: 10px;">{{ number_format($rapport['montant_en_retard'], 0, ',', ' ') }} F</small>
                    </div>
                </div>
            </div>
            <div class="col-md-2 col-sm-4 mb-2">
                <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #6f42c1 !important;">
                    <div class="card-body py-2 text-center">
                        ...
                        <small class="text-danger" style="font-size: 10px;">{{ number_format($rapport['montant_impaye'], 0, ',', ' ') }} F</small>
                    </div>
                </div>
            </div>
            <div class="col-md-2 col-sm-4 mb-2">
                <div class="card border-0 shadow-sm h-100" style="border-left: 4px solid #17a2b8 !important;">
                    <div class="card-body py-2 text-center">
                        ...
                        <br><small class="text-muted" style="font-size: 10px;">Taux: {{ $rapport['taux_paiement'] }}%</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        @if($rapport['prochaine_echeance'])
            <div class="alert alert-info py-2 mb-3" style="font-size: 13px;">
                ...
                @if($rapport['prochaine_echeance']['jours_restants'] >= 0)
                    <span class="badge bg-{{ $rapport['prochaine_echeance']['jours_restants'] <= 7 ? 'warning text-dark' : 'info' }} ms-2">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <h6 class="mb-0">
                                ...
                                {{ $rapportLocation['adresse'] }}
                                <small class="text-muted ms-2">{{ $rapportLocation['type_bien'] }}</small>
                            </h6>
                        </div>
                        <div class="col-md-4 text-end" style="font-size: 12px;">
                            <span class="me-2">
                                <strong>Loyer :</strong> {{ number_format($rapportLocation['loyer_mensuel'], 0, ',', ' ') }} F/mois
                            </span>
                            <span class="me-2">
                                <strong>Début :</strong> {{ $rapportLocation['date_debut'] ? $rapportLocation['date_debut']->format('d/m/Y') : '-' }}
                            </span>
                            ...
                            @endif
                            <span class="badge bg-{{ $rapportLocation['statut_location'] === 'actif' ? 'success' : ($rapportLocation['statut_location'] === 'resilie' ? 'danger' : 'warning') }}">
                                {{ ucfirst($rapportLocation['statut_location']) }}
                            </span>
                        </div>
                        <div class="col-md-2 text-end no-print">
                            <a href="{{ route('backend.locations.show', $location) }}" class="btn btn-outline-primary btn-sm" title="Voir la location">
